...
def LLM(frame, query):
    ...
    try:
        ...
        if any(key not in inputs or inputs[key].numel() == 0 for key in ["input_ids", "pixel_values"]):
            raise ValueError("Processor output contains empty tensors.")

        outputs = model.generate(**inputs, max_new_tokens=50)  # 控制生成长度
...
def extract_frames(video_path, frames_per_second=1):
    ...
    frame_interval = max(1, int(fps // frames_per_second))  # 确保帧间隔不为0
...
if __name__ == "__main__":
    ...
    try:
        ...
        raise RuntimeError(f"Failed to load BLIP-2 model on device {device}: {e}")

    video_folder = "/root/VideoTGB/inputs/videoinstruct/video"
    query_json_file = "/root/VideoTGB/inputs/videoinstruct/mp4_20_video.json"
    output_json_file = "/root/VideoTGB/inputs/videoinstruct/pseudo_label.json"
